...
- If the user uploads a PPT to use as a template/reference for creating a new presentation (e.g., "use this PPT as a template to make a new one", "create a PPT about xxx in this style"), follow the generation workflow in generate_slides.md (template mode) instead of this guide.
- If the user makes vague modification requests such as "beautify this PPT" or "adjust the design of this PPT", always follow the generation workflow in generate_slides.md instead of this guide.
...
### Case 1
- user query: Keep the text content word for word and make the PPT beautiful
- model actions: Do not follow the edit_user_slides.md flow; read generate_slides.md and follow the generation flow

### Case 2
- user query: This is an old version of my defense PPT. Read my latest version thesis and update the PPT content to match it
- model actions:
  1. Read the latest version thesis
  2. Use the read_file tool to read the PPT and get the PPT's text content (markdown format)
  3. Compare the PPT content with the thesis, analyze what needs to be changed (e.g., the PPT's literature review section doesn't match the thesis, the entire literature review chapter needs to be fully revised)
  4. Use the screenshot script to take screenshots of all pages in the relevant chapters, understand which text boxes and images are involved in this modification, and whether pages need to be added or removed
  5. Use the convert script to convert the PPTX to PPTD
  6. Read the relevant pages and modify the content
  7. Use the check script to validate and fix issues

### Case 3
- user query: Add a logo to the top-right corner of all content pages
- model actions:
  1. Use the read_file tool to read the PPT and get the PPT's text content (markdown format)
  2. Analyze the content to understand which pages are content pages
  3. Use the screenshot script to take screenshots of the relevant pages, analyze where a suitable blank space exists
  4. Use shell commands to batch add the logo to these pages
  5. View screenshots of some modified pages to verify the position and size are appropriate
