...
- When there is a conflict: If the user uploaded a PPT attachment AND selected a preset template, directly use the preset template selected by the user and enter the preset template path. The uploaded PPT attachment is only used as a content reference.
